...
public class BinaryTreeTopView3 {

    //We will use a TreeMap since treeMap stores elements in sorted order by keys
    //https://howtodoinjava.com/java-sorting-guide/

    public static Map<Integer, Node4> topView3(List<Node4> parentNode4s,
        ...
        Map<Integer, Node4> distanceNode4 = topView3(children, childrensDistanceFromRootMap);

        for (Map.Entry<Node4, Integer> each : childrensDistanceFromRootMap.entrySet()) {
            //Means that if anything already exists at a distance of each.getValue() in the distance Node4 map
            //Then replace it in the distance Node4 map. And if nothing exists then add it anyway.
            //Beacause we are looking from the top
    ...
    public static void printTop3(Node4 root) {
        if (root == null) return;
        ...
        distanceNode4Map = topView3(firstList, rootNode4DistanceMap);

        //We need to check here if the single one - root Node4 masks anything in the top view. Since
        // it is not coered by the recursive method above,
