...
public class PathBetweenTwoNodesConfusedVersion {
    ...
    public static List<TreeNode> getPath(TreeNode root, TreeNode someNode) {
        ...
        List p = getPath(root.left, someNode);
        if (p.isEmpty()) {
            p = getPath(root.right, someNode);
            if (p.isEmpty()) {
    ...
    //Finds path between a and b
    public static void getPathHelper(TreeNode root, TreeNode a, TreeNode b) {
        List<TreeNode> pathA = getPath(root, a);
        List<TreeNode> pathB = getPath(root, b);
        TreeNode previousSameNodeA = null, previousSameNodeB = null; int i = -1; int j = -1;
