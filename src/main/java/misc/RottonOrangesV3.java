...
public class RottonOrangesV3 {
	class Solution {

		/**
		 * This method will rot adjacent oranges and return a Set of coordinates of
		 * rotton oranges. HEre we are returning all of the rotten ones old and new. 
		 * We can skip returning the old rotten ones since we have already used them 
		 * to rot all possible oranges
		 */
		public Set<String> rotOranges(int[][] grid, Set<String> rottonCoordinates) {
			...
			Set<String> rottonAdj = new HashSet<>(); // This willbe of size 4
			for (String eachCoordinate : rottonCoordinates) {
				...
				if (rotThis(grid, right)) {
					rottonAdj.add(right[0] + ":" + right[1]);
				}
				if (rotThis(grid, left)) {
					rottonAdj.add(left[0] + ":" + left[1]);
				}
				if (rotThis(grid, top)) {
					rottonAdj.add(top[0] + ":" + top[1]);
				}
				if (rotThis(grid, bottom)) {
					rottonAdj.add(bottom[0] + ":" + bottom[1]);
				}
			}
			return rottonAdj;
		}
		...
		/* 
		 * Only one orange in a cell will rot
		 */
		private boolean rotThis(int[][] grid, int[] coordinate) {
			if (coordinate[0] == Integer.MIN_VALUE || coordinate[1] == Integer.MIN_VALUE)
				return false; // our of bound Indexes, Invalid.
		...
		/**
		 * This will be called only once for the first time
		 * 
		 * @param grid
		 * @return
		 */
		public Set<String> initRottonCoordinates(int[][] grid) {
		...
		public int orangesRotting(int[][] grid) {
			...
			Set<String> initialRotten = initRottonCoordinates(grid);
			
			Set<String> additionalRotten = new HashSet<String>();// No getter and setter so safe

			currentRottenSize = initialRotten.size();

			while (minute > 0) {
				previousRottenSize = currentRottenSize;

				// This is first minute
				// Instead of going over n X m grid again and again , we can keep adding
				// elements to the rotton set
				// by declaring rotton as global or pass rotten to alter it ( not good practice
				// to pass )
				additionalRotten = rotOranges(grid, initialRotten); 
				
				currentRottenSize = currentRottenSize + additionalRotten.size();
