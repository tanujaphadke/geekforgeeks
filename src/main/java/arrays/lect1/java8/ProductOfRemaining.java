...
public class ProductOfRemaining {
    ...
    public static int[] findProductOfRemaining3(int[] nums ){
        int [] leftProducts = IntStream.generate( ()->1).limit(nums.length).toArray() ;
        int [] rightProducts =IntStream.generate(()->1).limit(nums.length).toArray();
        int [] products =IntStream.generate(()->1).limit(nums.length).toArray();
...
//        }
        //Now, you shoudl just multiply left and right products.
...
//        }
        ...
        int rightProd = 1 ; // nums = [1, 2, 3, 4, 5]
        for( int i = nums.length-2 ; i >= 0; i -- ) {
            products[i] = products[i] * rightProd * nums[i+1];
